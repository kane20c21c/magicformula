...
def cells9(df: pd.DataFrame, tail: int | None = None) -> dict:
        ...
        for cv in VOL3:
            ...
            for cr in RET3:
                ...
                rows.append({"분기": str(b), "변동성": cv, "전분기 수익": cr,
                             "n": int(m.sum()),
                             "→다음분기 上": float((nxt[m] == "상").mean()),
                             "→다음분기 中": float((nxt[m] == "중").mean()),
                             "→다음분기 下": float((nxt[m] == "하").mean()),
                             "다음분기 순위백분위": float(pct[m].mean())})
    ...
    P = (D.groupby(["변동성", "전분기 수익"])
         .agg(분기수=("n", "size"), n합계=("n", "sum"), n평균=("n", "mean"),
              n최소=("n", "min"), n최대=("n", "max"),
              **{c: (c, "mean") for c in ("→다음분기 上", "→다음분기 中", "→다음분기 下")},
               다음분기_순위백분위=("다음분기 순위백분위", "mean")).reset_index()
         .rename(columns={"다음분기_순위백분위": "다음분기 순위백분위"}))
    ...
    for cv in VOL3[::-1]:
        for cr in RET3[::-1]:
            ...
            if not len(g):
                continue
            w = int((g["다음분기 순위백분위"] > 0.5).sum())
            sig_rows.append({"셀": f"변동성 {cv} · 전분기수익 {cr}", "분기": len(g),
                             "순위백분위>50% 인 분기": f"{w}/{len(g)}",
                             "평균 순위백분위": float(g["다음분기 순위백분위"].mean()),
...
def write_xlsx(out: Path, runs: list[tuple[str, pd.DataFrame, dict]],
    ...
    if C9:
        ...
        r += 1
        ax = C9["axis"]
        r = note(ws, r, "◆ 이 표를 읽는 법 — 첫 행(변동성 高 · 전분기수익 上)으로",
                 bold=True, size=12)
        for ln in [
            "   **한 행 = 한 셀.** 매 분기 전 종목을 (전분기 변동성 3등분) × "
            "(전분기 수익률 3등분) 으로 9칸에 나눠 담고, 14분기치를 쌓은 것이다.",
            "",
            "   **n합계 519** — 14분기 동안 이 칸에 들어간 **종목-분기 관측 수**. "
            "한 종목이 여러 분기에 반복 등장한다(같은 종목을 여러 번 센 값이다).",
            "   **전체 대비 18.5% · n평균 37.1** — 분기마다 평균 37종목이 이 칸에 있었다.",
            "",
            "   **→다음분기 上 / 中 / 下** = 이 칸의 종목들이 **다음 분기** 수익률 "
            "3등분에서 어디로 갔나. **세 값의 합은 항상 100%** 다.",
            "      36.2 / 25.3 / 38.5 → 다음 분기에 상위 36.2%, 중위 25.3%, 하위 38.5% 로 "
            "흩어졌다는 뜻. 무작위면 33.3 / 33.3 / 33.3 이다.",
            "      ⚠ 上(36.2)보다 下(38.5)가 크지만 **차이 2.3%p 는 잡음 범위**다"
            "(부호검정 p=0.791). 정작 눈에 띄는 건 **中이 25.3 으로 낮은 것**이고,",
            "        그게 '가운데를 비우고 양 끝으로 벌어진다' = 아래 ② 산포 결과다.",
            "",
            "   **다음분기 순위백분위 49.2%** — 이 칸 종목들의 **다음 분기 수익률 등수**를 "
            "그 분기 전체(약 200종목) 기준 백분위로 바꿔 평균낸 값.",
            "      100% = 그 분기 1등, 0% = 꼴찌, **50% = 딱 중간(무작위와 같음)**. "
            "49.2% 는 사실상 50% 다. 위 上/中/下 를 연속값 하나로 요약한 것이다.",
            "",
            "   **부호검정 p 0.791** — '이 칸이 다음 분기에 평균보다 잘하나' 를 "
            "**분기를 단위로** 검정한 값이다.",
            "      14분기 각각에서 이 칸의 순위백분위 평균을 구해 50%를 넘은 분기를 세니 "
            "**6/14**. 동전던지기(7/14)와 다르다고 할 수 없어 p=0.791.",
            "      ⚠ 종목 519개를 표본으로 세지 **않는다** — 같은 분기 종목들은 시장을 "
            "함께 타므로 독립이 아니다(유사반복).",
            "",
            "   실례 — 2025Q4 로 그룹을 짜고 2026Q1 을 본 분기: 이 칸에 44종목이 있었고 "
            "上 18 / 中 14 / 下 12 종목으로 갈렸다(순위백분위 평균 54.7%).",
            "      같은 칸 안에서 성호전자 +249.5%(순위 99.0%) 와 하이젠알앤엠 −41.2%"
            "(순위 0.5%) 가 함께 나왔다 — **한 칸이 이만큼 넓다.**",
            "",
        ...
        P = P[["변동성", "전분기 수익", "n합계", "전체 대비", "분기수", "n평균",
               "n최소", "n최대", "→다음분기 上", "→다음분기 中", "→다음분기 下",
               "다음분기 순위백분위"]]
        r = table(ws, P, r + 1,
                  {"n합계": "#,##0", "전체 대비": "0.0%", "분기수": "#,##0",
                   "n평균": "#,##0.0", "n최소": "#,##0", "n최대": "#,##0",
                   **{c: "0.0%" for c in ("→다음분기 上", "→다음분기 中",
                                          "→다음분기 下", "다음분기 순위백분위")}},
                  gcol="변동성", band=3)
