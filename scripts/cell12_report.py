...
def write_xlsx(out: Path, comp: pd.DataFrame, margin: pd.DataFrame,
    ...
    if P:
        ...
        DC, XC, LV = P.get("decomp"), P.get("decomp_cross"), P.get("level")
        if DC is not None:
            r = note(ws, r, "[F] 무엇을 재기에 안정적인가 — 성분 분해", bold=True, size=12)
            for ln in [
                "   YZ 는 세 성분의 합이고 **셋 다 2차 모멘트(분산·제곱)** 다: "
                "σ²_YZ = V(갭) + k·V(장중 드리프트) + (1−k)·V(장중 레인지).",
                "   2차 모멘트에는 **부호가 없다** — ±5% 로 움직이면 오르든 내리든 같은 값이다. "
                "즉 YZ 가 재는 것은 방향이 아니라 **하루 움직임의 크기(scale)** 하나뿐이다.",
                "   아래는 중첩 0 · 60일 블록에서 성분별로 따로 잰 지속성이다.",
            ]:
                r = note(ws, r, ln, size=9)
            r = put_table(ws, DC, r + 1,
                          fmt={"ρ(다음 블록)": "0.000", "ρ(한 블록 건너)": "0.000"},
                          signed={"ρ(다음 블록)", "ρ(한 블록 건너)"},
                          groups=[4, 4, 3], widths=[26, 14, 16])
            r += 1
            r = note(ws, r, "   같은 블록 안에서 서로 얼마나 같은 것을 재나", size=9, bold=True)
            r = put_table(ws, XC, r + 1, fmt={"ρ": "0.000"}, signed={"ρ"},
                          groups=[5, 2], widths=[40, 11])
            for ln in [
                "   읽기 — ⓐ **세 성분이 전부 지속적이다**(갭 0.66 · 드리프트 0.73 · 레인지 0.81). "
                "한 채널의 특성이 아니라 전방위다.",
                "   ⓑ **YZ 식이 특별해서가 아니다.** 단순 종가변동성 0.72 · 평균 |수익률| 0.77 · "
                "고저 레인지 0.83 으로 다 비슷하고, 블록 내 상관이 0.94~0.97 이다 —",
                "     **어떤 방식으로 재든 '하루 움직임의 크기' 는 지속적이다.** "
                "YZ 는 그걸 좀 더 정밀하게 잴 뿐이다.",
                "   ⓒ ★ **R² 가 왜 아닌지가 산수로 설명된다.** "
                "R² ≈ b²Var(x) / (b²Var(x) + σ²) 를 분자·분모로 쪼개면 —",
                "        **분모 σ(잔차 변동성) ρ = 0.498** → 지속적 (변동성이니까 당연하다)",
                "        **분자 |b|(기울기) ρ = 0.154** → 거의 지속 없음",
                "        그런데 R² 를 실제로 결정하는 건 분자다 — 블록 내 상관이 "
                "R²×|b| **+0.851** vs R²×잔차σ **−0.043**.",
                "     ⇒ **YZ 는 분모만 재고, R² 는 분자에 지배된다.** "
                "분모는 종목의 성질이고 분자는 그때그때의 국면이라, 두 축의 성격 차이가 여기서 갈린다.",
                "   ⓓ 시총은 ρ 0.984 로 거의 상수인데 YZ 와의 블록 내 상관은 **−0.366** 뿐이다 — "
                "YZ 의 지속성이 시총에서 오는 게 아니다.",
            ]:
                r = note(ws, r, ln, size=9)
            r += 1

        if LV:
            r = note(ws, r, "[G] 절대 수준 vs 순위 — 우리가 가르는 건 순위다", bold=True, size=12)
            for ln in [
                f"   시장 전체 YZ_60 중앙값은 {LV['mkt_min']:.4f}"
                f"({LV['mkt_min_at']:%Y-%m-%d}) → {LV['mkt_max']:.4f}"
                f"({LV['mkt_max_at']:%Y-%m-%d}) 로 **{LV['mkt_ratio']:.2f}배** 움직였다. "
                "절대 수준은 전혀 안정적이지 않다.",
                f"   그런데 그 최저일과 최고일(3.4년 간격) 사이에도 종목 **순위** 상관은 "
                f"ρ = {LV['rho_extremes']:.3f} 이고, 전·후반기로 나누면 {LV['rho_era']:.3f} 다.",
                "   ⇒ **시장 변동성이 통째로 오르내려도 '누가 더 흔들리는가' 의 서열은 유지된다.** "
                "우리 분할(그날 전 종목 중앙값)은 절대 수준이 아니라 이 서열을 가른다 — "
                "그래서 안정적으로 나오는 것이다.",
                f"   ⚠ 다만 {LV['rho_extremes']:.3f} 은 완전하지 않다 — 극단적으로 다른 두 시점을 "
                "비교하면 서열도 절반쯤은 뒤섞인다.",
            ]:
                r = note(ws, r, ln, size=9)
            r += 1

        CO, DY = P.get("comove"), P.get("decay")
        if CO and DY is not None:
            r = note(ws, r, "[H] 왜 서열이 남는가 — 변동성은 다 같이 움직인다",
                     bold=True, size=12)
            for ln in [
                "   '수준은 불안정한데 서열은 안정적' 이 성립하려면 변동성이 종목마다 따로가 아니라 "
                "**다 같이, 비례적으로** 움직여야 한다. 실제로 그런지 쟀다.",
                f"   (중첩 0 인 20일 블록 사이의 변화 Δlog(YZ) — {CO['n_tickers']}종목 × "
                f"{CO['n_periods']}구간. 롤링값 일간 차분을 쓰면 창이 겹쳐 공통 요인이 과대 추정된다.)",
                f"      · 종목 쌍 상관 평균 **{CO['pair_mean']:+.3f}** "
                f"(음수 쌍 {CO['pair_neg']:.1%}) · 제1주성분이 분산의 **{CO['pc1']:.1%}** "
                f"(제2 {CO['pc2']:.1%})",
                f"      · 시장 공통 변화는 구간마다 통째로 **±{100*(np.exp(CO['mkt_sd'])-1):.0f}%** "
                f"(최대 +{100*(np.exp(CO['mkt_max'])-1):.0f}% / 최소 "
                f"{100*(np.exp(CO['mkt_min'])-1):.0f}%)",
                f"      · **β>0 인 종목 {CO['beta_pos']:.1%}** — 시장 변동성이 내리면 "
                "**예외 없이 전 종목이 같은 방향**이다",
                f"      · β 중앙 **{CO['beta_med']:.2f}** (25~75% {CO['beta_q1']:.2f}~"
                f"{CO['beta_q3']:.2f}), 0.5~1.5 구간에 {CO['beta_near1']:.1%} — "
                "로그에서 β≈1 은 **평행 이동**, 즉 비례적 변화다",
                f"      · 종목별 R² 중앙 **{CO['r2_med']:.3f}** — 개별 변동성 변화의 "
                f"{CO['r2_med']:.0%} 가 공통분, 나머지 {1-CO['r2_med']:.0%} 가 종목 고유분",
                "   ⇒ **공통 요인이 방향을 정하고(β>0 100%) 크기도 대체로 비례한다** — "
                "그래서 수준이 통째로 2.66배 움직여도 서열이 남는다.",
                f"   ⇒ 동시에 **고유분 {1-CO['r2_med']:.0%} 가 서열을 조금씩 갈아엎는다** — "
                "그래서 서열이 '영구' 가 아니라 아래처럼 감쇠한다.",
            ]:
                r = note(ws, r, ln, size=9)
            r += 1
            r = note(ws, r, "   서열의 감쇠 (중첩 0 · 60일 블록, 간격별 순위상관)",
                     size=9, bold=True)
            r = put_table(ws, DY, r + 1,
                          fmt={"간격(거래일)": "#,##0", "≈개월": "#,##0",
                               "순위상관 ρ": "0.000", "블록쌍": "#,##0"},
                          groups=[len(DY)], widths=[14, 9, 12, 9])
            d0, dl = DY.iloc[0], DY.iloc[-1]
            for ln in [
                f"   ⚠ **'장기간 변하지 않는다' 는 과한 표현이다** — 3개월 "
                f"{d0['순위상관 ρ']:.3f} 에서 시작해 {int(dl['≈개월'])}개월 "
                f"{dl['순위상관 ρ']:.3f} 로 내려간다.",
                "     첫 6개월에 빠르게 섞이고(0.80→0.71) 이후 완만해져 0.57 근처에 머문다 — "
                "**'변하지 않는다' 가 아니라 '빠르게 절반쯤 섞이고 나머지가 오래 남는다'** 다.",
                "   ⚠ 뒤쪽 간격은 블록쌍이 8~10개뿐이라 잡음이 크다. 그리고 우리 표본은 "
                "**3.3년** 이므로 그 너머는 모른다.",
            ]:
                r = note(ws, r, ln, size=9)
            r += 1
