...
### 3.6-b 황금률 v3 승격 — 진입 신호 교체 (2026-08-25) `[실측]`

§3.6 의 진단을 실제로 반영했다. `COMBINED-v2-2026-05` → **`COMBINED-v3-2026-08`**.

| | v2 | v3 |
|---|---|---|
| 변동성 | BB×52주×레짐 점수표 | 52주위치 0.6 + BB꺾임 0.4 (레짐 미사용) |
| 모멘텀 | RSI 5-band 계단 | `(RSI−50)/5` 연속 선형 |
| 가중치 | T20/M20/Vu0/Va60 | **T0/M80/Vu0/Va20** |
| 임계 | 6.0 / 5.0 | 그대로 (실측이 지지) |

코어 67종목 상위 3% 20일 상대수익 **−1.29%(0/4) → +4.30%(4/4)**, p<0.001.
실운용(슬롯10·t+1시가·왕복0.43%) 누적 +118%→+400%, MDD −40.7%→−19.8%, Sharpe 0.96→1.87.
v2 대비 종목 순위상관 **−0.677** — 사실상 뒤집혔다(변동성 방향이 반대가 됐으므로).

검증 하네스를 새로 만들었다 — `scripts/validate_area_redesign.py`
(08-24 세션 분석 스크립트가 리포에 남아 있지 않았다. `build/rsi/compare/dist/
sweep/halves/port/thr/ic` 서브커맨드).
탐색용 별도 스크립트 = `scripts/redesign_from_pos52.py` (`axis/screen/combine/ops`).

**이번에 확인된 것 중 리포트와 어긋난 것** `[실측]`

1. 리포트 §14 **3순위(RSI 70~90 최고점 계단 조정)는 효과가 0** 이었다
   (+3.00% → +2.94~2.99%). 계단을 유지하는 한 어떤 배치도 소용없었고,
   **계단 자체를 없애야** +4.45% 가 나왔다.
2. 리포트 §8 의 "RSI≥90 구간 수익 −2.57%(n=140)" 는 재계산에서 **+2.07%(n=169)**
   로 부호가 반대였다. 전체의 0.1% 라 어느 쪽이든 무의미한 칸.
3. 리포트 §14 포트폴리오 표(2단구조 압도적)는 **매 20일 전량 재배치** 가정이라
   슬롯 운용이 반영되지 않았다. 슬롯 실운용으로 재면 2단구조 MDD −38.3% vs
   가중합 v3 −18.7% 로 **순위가 뒤집힌다**. 2단구조 누적의 대부분이 2026H1
   한 반기(+226%)에서 나오고 다음 반기에 시장만큼(−19.9%) 반납했다.
4. **종합점수 IC 는 v3 에서도 유의하지 않다** (+0.007, p=0.25). v2 가 유의하게
   음수(−0.015, p=0.0004)였던 것을 0 근처로 끌어올린 것이지 예측력 확보가 아니다.
   이 점수는 **예측 모델이 아니라 상위 선별기**로 취급해야 한다.

⚠ **v3 는 2026-07·08 두 달간 코어 신호가 0** 이다 (폭락 구간 → 현금). 고장이 아니다.
⚠ 롤백: `configs/active_strategy_v2.yaml` 복원 + `score_momentum_step()` ·
`score_volatility_table()` 로 교체 (두 함수 보존).
⚠ 생존편향·초강세장 한계는 그대로 — §5.1 순서는 여전히 유효하다.
...
✅ **진입 신호 교체는 2026-08-25 완료** (§3.6-b, v3 승격). 아래 ①~④ 는 **여전히
미완**이고, v3 의 성과 수치도 그 검증 전에는 확정값이 아니다. 남은 순서:
...
| 대상 | 기한 | 상태 |
|---|---|---|
| **황금률 v3 실운영 관찰** | ~2026-09-25 (전환 1개월) | `[실측]` 2026-08-25 승격. **2026-07·08 신호가 0 이라 당분간 관찰 표본이 안 쌓일 수 있다.** 첫 신호가 언제 나오는지, 나왔을 때 v2 대비 어떤 종목인지를 본다. v2 는 그림자로 굴리지 않기로 함 (Kane: A 먼저, B 나중에) |
| **B — 관측 순위표 설계** | v3 배포 안정화 후 | `[실측]` 보류 중. 낙폭(IC +0.032, 4/4) + Chaikin(+0.023, 4/4) 2축으로 재검토 필요 — **`Weis_Days` 는 파동 방향별로 나누면 유의성이 사라진다**(Dir=+1 p=0.53 / Dir=−1 p=0.22). 구성 확정 후 이름을 붙인다 |
...
| 항목 | 내용 |
|---|---|
| **LLV `Vol_Idx` 룩어헤드** | `[실측]` `stolab_data/indicator_calculator.py:229` 이 `g["Volume"].median()` — **전 구간 통계**로 나눈다. 과거 시점에서 알 수 없는 값이라 이 컬럼으로 만든 백테스트 수치는 전부 오염된다(20일 상대수익 IC −0.0365 가 이 때문에 나온 것). LLV 소관이라 별도 보고 필요. 황금률 v3 는 이 컬럼을 쓰지 않는다 |
